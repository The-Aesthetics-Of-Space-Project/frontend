      ...
      <div class="com-wrapper">
        <div class="thumnail-img-container">
          <div class="thumnail-img-wrapper">
            ...
            <button @click="modalOpen" v-if="hasImg" style="position: relative; top: -4em; left: 24em; width: 8%; height: 15%; border-radius: 50%; border: 1px solid darkslategrey; background-color: white;">
              <span v-html="svgIcon"/>
            </button>
        </div>
        <!-- 글 제목 작성-->
...
<script>
  ...
  export default {
    ...
    async created(){
      ...
      this.getArticlesId = parts.pop();

      if (this.getArticlesId) {
        await this.loadContestArticleData();
      }
    },
    computed: {
      imagePreview() {
      ...
      getArticleId(){
        return this.getArticlesId = this.$route.query.articleId;
      }
    },
    methods: {
